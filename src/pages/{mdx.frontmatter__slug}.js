...
export default function MarkdownTemplate({ data }) {
  ...
  const swapBookmarks = () => {
    ...
    if (bookmarkIcon === bookmarkChecked) {
      setBookmarks(
        bookmarks.filter((bookmark) => bookmark !== frontmatter.slug)
      );
  ...
  return (
    <>
      <Layout meta={meta} className="post-container">
        <div className="post-info">
          ...
          <div className="post-image">
            <img
              className={frontmatter.squishThumbnail ? "squish" : "no-squish"}
              src={`${origin}/assets/Banners/${frontmatter.articleThumbnail}`}
              alt="Thumbnail"
            ></img>
          </div>
          {/* Divider */}
      ...
      <div className="post-bookmark" onClick={swapBookmarks}>
        ...
        <span>
          {bookmarkIcon === bookmarkUnchecked
            ? "Bookmark?"
